...
func run() int {
	var (
		...
		).Default("/etc/web/httpserver/config/config.json").ExistingFile()
		tlsCertFile = kingpin.Flag(
			"tls.cert-file",
			"File containing the default x509 Certificate for HTTPS. (CA cert, if any, concatenated after server cert).",
		).Default("/etc/web/httpserver/certs/tls.crt").ExistingFile()
		tlsKeyFile = kingpin.Flag(
			"tls.private-key-file",
			"File containing the default x509 private key matching --tls.private-key-file.",
		).Default("/etc/web/httpserver/certs/tls.key").ExistingFile()
	)
	...
	// add version info
	injectionVersion()
	kingpin.Version(version.Print("go-httpserver"))
	kingpin.HelpFlag.Short('h')
	kingpin.Parse()

	logger := promlog.New(promlogConfig)
	level.Info(logger).Log("msg", "Starting go-httpserver", "version", version.Info())
